class ErasController < ApplicationController
  ...
  # GET /eras.json
  def index
      @eras = Era.all.order("RANDOM()")
      ids = @eras.map do |era| era.id end
      
      if ids == [1,2,3] then @eras = Era.order('end_year DESC') end
        # varmistetaan ettei tule oikeaa tulosta heti alkuun
  end
  ...
  private
    ...
    def era_params
      params.require(:era).permit(:name, :start_year, :end_year)
    end
end
